...
if(!class_exists("ThemeControlHelper") ) {	
	...
	class ThemeControlHelper extends Controller{
		...
		public function setThemeDir( $dir ){
			$this->_themeDir = $dir; 
			$this->_themeScssDir = $dir.'/development/scss/';
		}
		...
		private function autoLoadThemeCss(){
			...
			/* load global and defaul stylesheets file */
				...
				if( $this->getDirection() == 'rtl' ) {
					if( is_file($this->_themeDir . '/stylesheet/skins/'.$this->skin.'/bootstrap-rtl.css') ){
			 	 		$this->addCss( $this->themeURL.'skins/'.$this->skin.'/bootstrap-rtl.css' );
			 		}else {		
						$this->addCss( $this->themeURL.'bootstrap-rtl.css' ); 
					}
				} else {
					if( is_file($this->_themeDir . '/stylesheet/skins/'.$this->skin.'/bootstrap.css') ){
			 	 		$this->addCss( $this->themeURL.'skins/'.$this->skin.'/bootstrap.css' );
			 		}else {		
			...
			$skinfont = false;
			if( $this->skin ){
			 	$this->addCss( $this->themeURL.'skins/'.$this->skin.'.css' );	
			} else {
				$this->addCss( $this->themeURL.'stylesheet.css' ); 
			} 
	 	...
	 	public function loadModulesByLayout( $layout_id ){
	 		...
	 		foreach( $modules as $module ){
				...
				if (isset($part[0]) && $this->config->get($part[0] . '_status')) {
					$modulesList[$module['position']][] = $this->load->controller('module/' . $part[0]);
				}
				...
				if (isset($part[1])) {
					...
					if ($setting_info && $setting_info['status']) {
						$modulesList[$module['position']][] = $this->load->controller('module/' . $part[0], $setting_info);
					}
				}
	 		}
		...
		public function renderModule( $module, $args = array() ){ 
				return $this->load->controller('module/' . $module);
			return ;
		}	
