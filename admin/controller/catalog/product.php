...
class ControllerCatalogProduct extends Controller {
	...
	public function mapParentCategories() {
		$this->load->model('catalog/category');

		$map = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category")->rows;
		...
		foreach ($map as $value) {
			...
			if ($category['parent_id'] != 0 || $category['parent_id'] != null) {
				echo "mapping " . $product_id . " to " . $category['parent_id'] . "<br>";
				$this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "product_to_category SET product_id = '" . (int)$product_id . "', category_id = '" . (int)$category['parent_id'] . "'");
			}
	 	} 
	}
	...
	public function setSeoUrl () {
		...
		foreach ($results as $product) {
			...
			if ($product_info['keyword'] == null || strpos($product_info['keyword'], 'html') !== false) {
				echo $product_info['keyword'] . '<br>';
				$keyword = str_replace(array('\'', '"', '`', ' ', '_', '&', '/', '\\', '-', ',', ':', ';'), '-', strtolower($product_info['name'] . "-" . $product_info['model']));
				$keyword = str_replace(array('---', "--"), "-", $keyword);
				$keyword = str_replace(array('ç', 'Ç', 'ı', 'İ', 'ü', 'Ü', 'Ğ', 'ğ', 'Ş', 'ş', 'Ö', 'ö'), array('c', 'c', 'i', 'i', 'u', 'u', 'g', 'g', 's', 's', 'o', 'o'), $keyword);

				$query = "product_id=" . $product['product_id'];
				$this->db->query('insert into oc_url_alias set query="' . $query . '", keyword="' . $keyword . '"');
			}
		}
	}
	...
	public function setRelatedProducts () {
		...
		foreach ($products as $product) {
			$filters = array(
				'filter_category' => $product['category_id'],
				'filter_status'	=> 1,
				'limit' 				=> 8,
				'sort'				=> 'RAND'
			);

			$related_products = $this->model_catalog_product->getProducts($filters);

			foreach ($related_products as $related) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_related WHERE product_id = '" . (int)$product['product_id'] . "' AND related_id = '" . (int)$related['product_id'] . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_related SET product_id = '" . (int)$product['product_id'] . "', related_id = '" . (int)$related['product_id'] . "'");
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_related WHERE product_id = '" . (int)$related['product_id'] . "' AND related_id = '" . (int)$product['product_id'] . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_related SET product_id = '" . (int)$related['product_id'] . "', related_id = '" . (int)$product['product_id'] . "'");
			}
		}
	}

	public function setSeoUrlCategories() {
		$this->load->model('catalog/category');
		...
		foreach ($results as $category) {
			...
			if ($category_info['keyword'] == null) {
				$keyword = str_replace(array('\'', '"', '`', ' ', '_', '&', '/', '\\', '-', ',', ':', ';'), '-', strtolower($category_info['name']));
				$keyword = str_replace(array('---', "--"), "-", $keyword);
				$keyword = str_replace(array('ç', 'Ç', 'ı', 'İ', 'ü', 'Ü', 'Ğ', 'ğ', 'Ş', 'ş', 'Ö', 'ö'), array('c', 'c', 'i', 'i', 'u', 'u', 'g', 'g', 's', 's', 'o', 'o'), $keyword);
	...
	{
		return false;
		require_once(DIR_SYSTEM . 'external/PHPExcel.php');
		$this->load->model('catalog/category');
		$this->load->model('catalog/manufacturer');
		
		$file = DIR_STORAGE . 'test.xls';
		...
		for ($row = 10; $row <= $highestRow; $row++) {
			...
			/*$category_info = $this->db->query("SELECT * FROM " . DB_PREFIX ."category_description WHERE name COLLATE UTF8_GENERAL_CI LIKE '%" . $category . "%' LIMIT 1" )->row;

			if ($category_info) {
				echo "Added: " . $product_id . "<br>";
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category SET product_id = " . $product_id . ", category_id = " . $category_info['category_id'] . "");
			} else {
				echo "New Cat<br>";
				echo $category;
				...
				$category_info = $this->db->query("SELECT * FROM " . DB_PREFIX ."category_description WHERE name LIKE '%" . $product_name . "%' LIMIT 1" )->row;

				if ($category_info) {
					echo "Added: " . $product_id . "<br>";
					$this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "product_to_category SET product_id = " . $product_id . ", category_id = " . $category_info['category_id'] . "");
				}
			}*/

			$man_info = $this->db->query("SELECT * FROM " . DB_PREFIX ."manufacturer WHERE name COLLATE UTF8_GENERAL_CI LIKE '%" . $manufacturer . "%' LIMIT 1" )->row;
			if ($man_info) {
				echo $manufacturer . "<br>";
				$this->db->query("UPDATE " . DB_PREFIX . "product set manufacturer_id = " . $man_info['manufacturer_id'] . " where product_id = " . $product_id);
			} else {
				echo "New Man<br>";
				echo $manufacturer . "<br>";
	...
	public function imageDownloader ()
	{
		...
		foreach ($results as $key => $value) {
			$image = "http://www.buyuklereoyuncaklar.com.tr/image/" . $value['image'];
			$folders = explode('/', $value['image']);
			array_pop($folders);

			$base = '';
			foreach ($folders as $key => $string) {
				if ($key == 0)
				{
					$base = '/var/www/html/buyuklereoyuncaklar.com.tr/image/catalog/' . $string . "/";
				}
				else
				{
					$base .= $string . '/';
			...
			file_put_contents("/var/www/html/buyuklereoyuncaklar.com.tr/image/catalog/" . $value['image'], file_get_contents($image));
		}
	}
}
