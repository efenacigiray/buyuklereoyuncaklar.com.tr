...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		// Get files
		$files = glob($directory . '/' . $filter_name . '*.{jpg,jpeg,png,gif,JPG,JPEG,PNG,GIF,ico}', GLOB_BRACE);
		...
		$data['multi_upload'] = $this->url->link('common/filemanager/multiupload', 'token=' . $this->session->data['token'] . isset($url) ? $url : '', 'SSL');
			...
			foreach ($files as $file) {
				...
				if (!empty($file['name']) && is_file($file['tmp_name'])) {
					...
					if (!in_array(utf8_strtolower(utf8_substr(strrchr($filename, '.'), 1)), $allowed)) {
						$error = true; 
						$json['error'][] = $filename . " - " . $this->language->get('error_filetype');
					}
					...
					if ($file['error'] != UPLOAD_ERR_OK) {
						$error = true; 
						$json['error'][] = $filename . " - " . $this->language->get('error_upload_' . $file['error']);
					}
