...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if ($category_id == 95)
			$this->response->redirect($this->url->link('product/special', '', 'SSL'));
		...
		$data = $this->cache->get('category_' . $category_id . "_page_" . $page );
				...
				if ($category_info) {
					...
					foreach ($results as $result) {
						...
						if ($result['image'] && !empty($result['image'])) {
							...
							$thumb = $this->model_tool_image->resize(NO_IMAGE, $this->config->get('config_image_category_width'), $this->config->get('config_image_category_height'));
						}
					...
					if (count($data['categories']) == 4) {
						$data['subspan'] = 'col-md-3';
					} else if (count($data['categories']) == 3) {
						$data['subspan'] = 'col-md-4';
					} else if (count($data['categories']) == 2) {
						$data['subspan'] = 'col-md-6';
					} else if (count($data['categories']) == 1) {
						$data['subspan'] = 'col-md-12';
					} else {
						$data['subspan'] = 'col-md-3';
					}
					...
					$limits = array_unique(array($this->config->get('config_product_limit'), 30, 60, 90, 120));
					...
					$this->cache->set('category_' . $category_id . "_page_" . $page , $data);
				}
			}
