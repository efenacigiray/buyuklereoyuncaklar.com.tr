...
<div class="<?php echo $additional_class; ?> box search_block">
	...
	<div id="search<?php echo $module ?>" class="search pull-left">
		<input type="text" name="search" autocomplete="off" placeholder="<?php echo $objlang->get("text_search");?>" value="" class="input-search form-control">
		<span class="button-search fa fa-search"></span>
	</div>
	<input type="hidden" name="sub_category" value="1" class="sub_category"/>
	<input type="hidden" name="route" value="product/search"/>
	<input type="hidden" name="description" value="true" class="description"/>
	</form>
	...
	$(selector).find('input[name=\'search\']').autocomplete({
		...
		$(selector).find('input[name=\'search\']').data( "autocomplete" )._renderItem = function( ul, item ) {
			...
			if(show_price){
				...
				html += '<div class="price"><?php echo $objlang->get("text_price");?>'+item.price;	
			}
