...
<div class="pending-modules col-sm-3">
    ...
    <div class="unhookpos panel panel-default" data-position="unhookpos">
        ...
        <div class="panel-body">
            <?php  foreach( $unhookmodules as $uc =>  $umods ){  if($uc =='themecontrol'){ continue; } ?>
            <div class="panel-modules   edit-container" data-position="unhookpos">

                <?php  $i =1; foreach( $umods['module'] as $umod ){  $umodsname = $umod['name'];  ///echo '<pre>'.print_r( $umod['name'],1 );die;?>
                <div class="module-pos" data-position="unhookpos" data-id="<?php echo $umod['code'];?>">
                    <div class="edit-module">
                        <a target="_blank" href="<?php echo $ourl->link('module/'.preg_replace( "#(\.[^.]+)#", "", $umod['code']),'token='.$token);?>"><b><?php echo $umodsname; ?></b></a>
                    </div>
                </div>
...
<div class="slideshow  edit-container clearfix" data-position="mainmenu">
    ...
    <div class="module-pos <?php if( !$modulepos['status']){ ?>mod-disable<?php } ?>" data-position="mainmenu" data-id="<?php echo $modulepos['index'];?>">
        <div class="edit-module">
            <a target="_blank" href="<?php echo $ourl->link('module/'.$modulepos['code'],'token='.$token);?>"><b><?php echo $modulepos['title']; ?></b></a>
        </div>
    </div>
    <?php } }?>
</div>
...
<div class="outsite_left  edit-container clearfix"  data-position="outsite_left">
    <div class="pos">Out Site left</div>
    <?php if( isset($layout_modules['outsite_left']) ){
				foreach( $layout_modules['outsite_left'] as $modulepos ) {
			?>
    <div class="module-pos <?php if( !$modulepos['status']){ ?>mod-disable<?php } ?>" data-position="outsite_left" data-id="<?php echo $modulepos['index'];?>">
        <div class="edit-module">
...
<div class="outsite_right  edit-container clearfix"  data-position="outsite_right">
    <div class="pos">Out Site Right</div>
    <?php if( isset($layout_modules['outsite_right']) ){
				foreach( $layout_modules['outsite_right'] as $modulepos ) {
			?>
    <div class="module-pos <?php if( !$modulepos['status']){ ?>mod-disable<?php } ?>" data-position="outsite_right" data-id="<?php echo $modulepos['index'];?>">
        <div class="edit-module">
            <a target="_blank" href="<?php echo $ourl->link('module/'.$modulepos['code'],'module_id='.$modulepos['module_id'].'&token='.$token);?>"><b><?php echo $modulepos['title']; ?></b></a>
        </div>
    </div>
